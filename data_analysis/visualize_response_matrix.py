...
from dynamic_models.temporal_eval.data_filter import DEFAULT_FILTER, filter_data
from dynamic_models.temporal_eval.data_loader import load_student_split_data, load_unified_data
...
def plot_avg_improvement(corr, row_indices, col_indices, output_path, raw_corr=None):
    """Line plot of average score across attempts: filtered vs all (unfiltered) data."""
    snapshots = get_attempt_snapshots(corr, max_attempt=10)
    attempts = list(range(1, 11))

    if raw_corr is None:
        raw_corr = corr
    raw_snapshots = get_attempt_snapshots(raw_corr, max_attempt=10)
    all_students = np.arange(raw_corr.shape[0])
    all_items = np.arange(raw_corr.shape[1])

    filtered_scores = []
    all_scores = []
    for snap_f, snap_a in zip(snapshots, raw_snapshots):
        sub_f = snap_f[np.ix_(row_indices, col_indices)]
        valid_f = sub_f[sub_f != -1]
        filtered_scores.append(valid_f.mean() if len(valid_f) > 0 else 0.0)

        sub_a = snap_a[np.ix_(all_students, all_items)]
    ...
    ax.set_ylabel("Average Score (pass rate)", fontsize=14)
    ax.set_title("Average Score by Attempt", fontsize=16)
    ...
    for i, (sf, sa) in enumerate(zip(filtered_scores, all_scores)):
        ax.annotate(f"{sf:.3f}", (attempts[i], sf), textcoords="offset points",
                    xytext=(0, 10), ha="center", fontsize=10, color="#1f77b4")
...
def plot_attempt_progression_compact(corr, matrix, qi, row_indices, col_indices, selected_qs, output_path):
    ...
    panel_w = max(7, min(n_cols * 0.035, 18))
    panel_h = max(12, min(n_rows * 0.05, 50))
    ...
    for idx, a in enumerate(attempt_nums):
        ...
        ax.set_title(f"Attempt {a}", fontsize=40, pad=16)
        ax.tick_params(axis="both", labelsize=28)

    fig.supxlabel("Unit tests (chronological)", fontsize=44, y=0.01)
    fig.supylabel("Students", fontsize=44, x=0.005)
    ...
    ], loc="lower right", ncol=3, fontsize=36, bbox_to_anchor=(1.0, -0.01),
       frameon=False)

    fig.tight_layout(rect=[0.025, 0.045, 1, 1])
    os.makedirs(os.path.dirname(output_path), exist_ok=True)
    fig.savefig(output_path, dpi=300, bbox_inches="tight")
    plt.close(fig)
    print(f"Plot saved: {output_path}")
...
def main():
    ...
    if args.mode == "student":
        ...
        data = load_unified_data(args.course)
        data = filter_data(data, DEFAULT_FILTER)
        row_indices = np.arange(data.n_students)
        col_indices = np.arange(data.n_items)
        qi = data.question_infos
        corr = data.correctness_matrix
        selected_qs = sorted(qi["qidx"].unique())

    raw_data = load_unified_data(args.course)
    raw_corr = raw_data.correctness_matrix
    ...
    plot_attempt_progression_compact(corr, matrix, qi, row_indices, col_indices, selected_qs, compact_path)

    improvement_path = os.path.join(output_dir, "avg_improvement_by_attempt.png")
    plot_avg_improvement(corr, row_indices, col_indices, improvement_path, raw_corr=raw_corr)
