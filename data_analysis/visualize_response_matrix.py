...
def plot_attempt_progression_compact(corr, matrix, qi, row_indices, col_indices, selected_qs, output_path):
    ...
    fig, axes = plt.subplots(n_panel_rows, n_panel_cols, figsize=(22.5, 6), sharey=True, sharex=True)
    ...
    for idx, a in enumerate(attempt_nums):
        ...
        ax.set_title(f"Attempt {a}", fontsize=26, pad=10)
        ax.tick_params(axis="y", labelsize=20)

    fig.supxlabel("Unit tests (chronological)", fontsize=28, y=0.01)
    fig.supylabel("Students", fontsize=28, x=0.005)
    ...
    ], loc="lower right", ncol=3, fontsize=26, bbox_to_anchor=(1.0, -0.01),
       frameon=False)

    fig.tight_layout(rect=[0.012, 0.07, 1, 1], h_pad=1.0)
