...
def load_data(data_folder, smoke_test, test_split=0.2):
    ...
    with open(f"{result_folder}/{args.course_name}_seed{args.seed}/student_idxs.pkl", "wb") as f:
        pickle.dump(student_idxs, f)

    # Save student saidx indexes
    with open(f"{result_folder}/{args.course_name}_seed{args.seed}/list_saidx.pkl", "wb") as f:
        pickle.dump(list_saidx, f)

    # Save all squidx indexes
    with open(f"{result_folder}/{args.course_name}_seed{args.seed}/all_squidx.pkl", "wb") as f:
        pickle.dump(all_squidx, f)

    # Save list of available student indexes
    with open(f"{result_folder}/{args.course_name}_seed{args.seed}/list_available_sidx.pkl", "wb") as f:
    ...
    with open(f"{result_folder}/{args.course_name}_seed{args.seed}/list_saidx2aidx.pkl", "wb") as f:
