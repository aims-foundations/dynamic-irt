...
def prepare_data(csv_path: str) -> tuple[dict, pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    ...
    # Subsample for quick run - keep students with max time < 100, then sample 100
    student_max_time = df.groupby("student_id")["time_index"].max()
    eligible_students = student_max_time[student_max_time < 100].index.tolist()
    print(f"Students with T_max < 100: {len(eligible_students)} / {df['student_id'].nunique()}")

    if len(eligible_students) < 100:
        print(f"  Warning: only {len(eligible_students)} eligible students (need 100). Using all.")
        sampled_students = eligible_students
    else:
        rng = np.random.default_rng(seed=42)
        sampled_students = rng.choice(eligible_students, size=100, replace=False).tolist()
