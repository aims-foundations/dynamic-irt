...
if __name__ == "__main__":
    ...
    list_saidx = pickle.load(open(f"results/{args.course_name}_seed{args.seed}/list_saidx.pkl", "rb"))
    student_thetas = pickle.load(open(f"results/{args.course_name}_seed{args.seed}/student_{picked_sidx}_thetas.pkl", "rb"))
    # >>> n_sample x n_time
    ...
    theta_prior = get_theta_priors(torch.tensor(student_times).reshape(-1, 1)).to(device)
    with torch.no_grad():
        theta_prior.eval()
    ...
    plt.savefig(f"plots/student_{picked_sidx}_theta_prior_plot_seed{args.seed}.png", dpi=300)
    plt.close()
