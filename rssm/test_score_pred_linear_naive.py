...
def train(
    ...
    torch.save(
        scorer.state_dict(), f"{args.ckpt_folder}/{args.cls}/naive_linear_scorer.pth"
    )
...
if __name__ == "__main__":
    ...
    if os.path.exists(f"{args.ckpt_folder}/{args.cls}/naive_linear_scorer.pth"):
        print("Loading pre-trained model")
        scorer.load_state_dict(
            torch.load(f"{args.ckpt_folder}/{args.cls}/naive_linear_scorer.pth")
        )
    print("Scorer:", scorer)
    ...
    if args.test_only:
        ...
        with open(
            f"eval_results/{args.cls}/naive_linear_scorer_train_res.json", "w"
        ) as f:
            json.dump(train_eval_res, f)
        with open(
            f"eval_results/{args.cls}/naive_linear_scorer_test_res.json", "w"
        ) as f:
