...
import torch
from models import NaiveScorer
...
def train(
    ...
    for epoch in tqdm(range(args.epochs)):
        ...
        for i in range(0, n_students, args.batch_size):
            ...
            list_score_hat = scorer(ques_embs, tcs_embs)
    ...
    plt.ylabel("Loss")
    plt.legend()
    plt.savefig("plots/naive_score_train_losses.png")
    plt.close()
    ...
    plt.ylabel("Accuracy")
    plt.legend()
    plt.savefig("plots/naive_score_test_acc.png")
    plt.close()

    # Save model
    ensure_dir(f"{args.ckpt_folder}/{args.cls}")
    torch.save(scorer.state_dict(), f"{args.ckpt_folder}/{args.cls}/naive_scorer.pth")
...
if __name__ == "__main__":
    ...
    scorer = NaiveScorer(input_dim=emb_dim, hidden_dim=hidden_dim).to(device)

    # Load pre-trained model
    if os.path.exists(f"{args.ckpt_folder}/{args.cls}/naive_scorer.pth"):
        print("Loading pre-trained model")
        scorer.load_state_dict(
            torch.load(f"{args.ckpt_folder}/{args.cls}/naive_scorer.pth")
        )
    print("Scorer:", scorer)
    ...
    if args.test_only:
        ...
        with open(f"eval_results/{args.cls}/naive_scorer_train_res.json", "w") as f:
            json.dump(train_eval_res, f)
        with open(f"eval_results/{args.cls}/naive_scorer_test_res.json", "w") as f:
