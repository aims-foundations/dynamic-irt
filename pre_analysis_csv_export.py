...
for filename in os.listdir(directory):
    if filename.endswith('.json'):
        ...
        with open(file_path, 'r') as file:
            ...
            flat_df['time'] = pd.to_datetime(flat_df['time'], format='%d/%m/%y, %H:%M:%S')
            ...
            filtered_flat_df['time'] = pd.to_datetime(filtered_flat_df['time'], format='%d/%m/%y, %H:%M:%S')
            filtered_flat_df = filtered_flat_df.sort_values(['id', 'time'])
            filtered_flat_df['time_delta'] = filtered_flat_df.groupby('id')['time'].diff()
            ...
            filtered_df_name = f"filtered_df_{filename.replace('.json', '').replace('-', '_')}"
