...
def format_dataset(repo_id, threshold=0.8):
    question_attempts = []
    student_id_to_index = {}
    correctness = []
    all_questions = []
                    ...
                    with open(file_path, "r") as json_file:
                        ...
                        for answer in data["student_answers"]:
                            ...
                            if student_id not in student_id_to_index:
                                student_id_to_index[student_id] = len(
                                    student_id_to_index
                                )
                ...
                try:
                    with open(file_path, "r") as json_file:
                        ...
                        for answer in data["student_answers"]:
                            ...
                            for response_history in answer["response_history"]:
                                current_question, question_max_score = (
                                    question_content_map.get(
                                        response_history["question"], ("", 0)
                                    )
                                )
                                if (
                                    not current_question
                                    or float(question_max_score) == 0
                                ):
                                    continue

                                q_idx = None
                                for idx, q_unique in enumerate(
                                    unique_questions_by_distance
                                ):
                                    if (
                                        edit_distance(
                                            tokenize(current_question), q_unique
                                        )
                                        > threshold
                                    ):
                                        q_idx = idx
                                ...
                                for t, result in enumerate(
                                    ...
                                    if result["marks"] == "":
                                        score = 0
                                    elif float(result["marks"]) > float(
                                        question_max_score
                                    ):
                                        break
                                    else:
                                        score = float(result["marks"]) / float(
                                            question_max_score
                                        )

                                    response = ""
                                    if result["action"].startswith("Prechecked"):
                                        response = result["action"].replace(
                                            "Prechecked: ", ""
                                        )
